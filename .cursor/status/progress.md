...
| 2026-07-20 | #25 | **GOVERNANCE/COMPLIANCE COMPREHENSIVE FIX.** Root cause: Classification.DatasetID (string) and other cols were NULL in DB causing sqlx scan errors, making loadClassifications() silently return empty → 0 classifications → 0 recommendations. Fix: loadClassifications() helper with COALESCE for all nullable string columns and ::text casts for nullable uuid columns. Expanded PIITypes map to include PERSON_NAME, PERSON, IP_ADDRESS, CREDIT_CARD etc. Added DerefStr helper. Added compliance_assessments table (migration 015) + GET /advisor/assessment/logs endpoint. Frontend: assessment history panel added to advisor page. All 8 features verified: Run Assessment ✅ (26 classifications, 20 findings) Recommendations ✅ Assessment Logs ✅ Compliance Gaps ✅ Policy Evaluation ✅ Defense Docket ✅ Playbooks ✅ Risk Score ✅ |
